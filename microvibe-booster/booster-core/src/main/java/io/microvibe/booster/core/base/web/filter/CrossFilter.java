...
public class CrossFilter implements Filter {
	...
	public void doFilter(final ServletRequest servletRequest,
		...
		CrossFilter.CORSRequestType requestType = checkRequestType(request);

		// Adds CORS specific attributes to request.
		if (decorateRequest) {
			CrossFilter.decorateCORSProperties(request, requestType);
		}
		switch (requestType) {
			case SIMPLE:
	...
	protected void handleSimpleCORS(final HttpServletRequest request,
		...
		CrossFilter.CORSRequestType requestType = checkRequestType(request);
		if (!(requestType == CrossFilter.CORSRequestType.SIMPLE ||
			requestType == CrossFilter.CORSRequestType.ACTUAL)) {
			throw new IllegalArgumentException("corsFilter.wrongType");
		}

		final String origin = request
			.getHeader(CrossFilter.REQUEST_HEADER_ORIGIN);
		...
		if (anyOriginAllowed && !supportsCredentials) {
			...
			response.addHeader(
				CrossFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN,
				"*");
		} else {
		...
		if (supportsCredentials) {
			response.addHeader(
				CrossFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_CREDENTIALS,
				"true");
		}
		...
		if ((exposedHeaders != null) && (exposedHeaders.size() > 0)) {
			...
			response.addHeader(
				CrossFilter.RESPONSE_HEADER_ACCESS_CONTROL_EXPOSE_HEADERS,
				exposedHeadersString);
		}

		// Indicate the response depends on the origin
		response.addHeader(CrossFilter.REQUEST_HEADER_VARY,
			CrossFilter.REQUEST_HEADER_ORIGIN);
	...
	protected void handlePreflightCORS(final HttpServletRequest request,
		...
		String accessControlRequestMethod = request.getHeader(
			CrossFilter.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_METHOD);
		...
		String accessControlRequestHeadersHeader = request.getHeader(
			CrossFilter.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_HEADERS);
		...
		// Section 6.2.7
		if (supportsCredentials) {
			response.addHeader(CrossFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, origin);
			response.addHeader(CrossFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_CREDENTIALS, "true");
		} else {
			if (anyOriginAllowed) {
				response.addHeader(CrossFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, "*");
			} else {
				response.addHeader(CrossFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, origin);
			}
		}
		...
		if (preflightMaxAge > 0) {
			response.addHeader(CrossFilter.RESPONSE_HEADER_ACCESS_CONTROL_MAX_AGE, String.valueOf(preflightMaxAge));
		}

		// Section 6.2.9
		response.addHeader(CrossFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_METHODS, accessControlRequestMethod);

		// Section 6.2.10
		if ((allowedHttpHeaders != null) && (!allowedHttpHeaders.isEmpty())) {
			response.addHeader(CrossFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_HEADERS, join(allowedHttpHeaders, ","));
		}
	...
	private void handleInvalidCORS(final HttpServletRequest request,
								   final HttpServletResponse response, final FilterChain filterChain) {
		String origin = request.getHeader(CrossFilter.REQUEST_HEADER_ORIGIN);
		...
		switch (corsRequestType) {
			case SIMPLE:
				request.setAttribute(
					CrossFilter.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST,
					Boolean.TRUE);
				request.setAttribute(CrossFilter.HTTP_REQUEST_ATTRIBUTE_ORIGIN,
					request.getHeader(CrossFilter.REQUEST_HEADER_ORIGIN));
				request.setAttribute(
					CrossFilter.HTTP_REQUEST_ATTRIBUTE_REQUEST_TYPE,
			...
			case PRE_FLIGHT:
				...
				request.setAttribute(
					CrossFilter.HTTP_REQUEST_ATTRIBUTE_REQUEST_HEADERS, headers);
				break;
			case NOT_CORS:
				request.setAttribute(
					CrossFilter.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST,
				...
				if (method != null) {
					if ("OPTIONS".equals(method)) {
						...
						if (mediaType != null) {
							...
							// 不强制要求ContentType头信息,fix@2017/10/27
							requestType = CORSRequestType.SIMPLE;
						}
					} else {
						requestType = CORSRequestType.ACTUAL;
